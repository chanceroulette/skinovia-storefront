...
export async function createCheckout(variantId, quantity = 1) {
  ...
  // so Shopify's checkoutUrl on that host 404s. Route checkout to the dedicated
  // Shopify-served subdomain (checkout.skinovia.ae) so checkout actually loads.
  const CHECKOUT_HOST = process.env.SHOPIFY_CHECKOUT_DOMAIN || 'checkout.skinovia.ae';
  try {
    const u = new URL(url);
    u.protocol = 'https:';
    u.host = CHECKOUT_HOST;
    url = u.toString();
  } catch {}
  return url;
}
